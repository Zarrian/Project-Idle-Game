...
public class ManagerStatistiques : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // Repart d'une base propre : si ce composant a d�j� �t� activ� une
        // fois (r�-activation d'un panel UI par ex.), on �vite de compter en
        // double les vaisseaux d�j� pr�sents dans les listes.
        allships.Clear();
        shipPlayer.Clear();
        shipInvaders.Clear();

        // R�cup�re tous les vaisseaux d�j� pr�sents et actifs dans la sc�ne
    ...
    private void OnDisable()
    {
        ...
        // Sans �a, une r�activation ult�rieure relance UpdateDamage/UpdateUI
        // par-dessus les chaines d�j� en cours : chaque cycle enable/disable
        // empilerait deux boucles infinies suppl�mentaires qui ne s'arr�tent
    ...
    /// <summary>
    /// Lerp exponentiel vers targetValue, ind�pendant du framerate. Contrairement
    /// � Mathf.Lerp(bar.fillAmount, target, vitesse * Time.fixedDeltaTime) � un
    /// pi�ge classique � cette formule donne le M�ME r�sultat visuel peu importe
    /// le framerate/le fixedDeltaTime, parce qu'elle compose correctement le
    /// taux de rattrapage sur plusieurs frames au lieu de l'additionner lin�airement.
    /// </summary>
    ...
    {
        // while(true) au lieu d'un StartCoroutine(UpdateUI()) r�cursif en fin
        // de m�thode : une seule coroutine vit pour toute la dur�e, au lieu
        // d'en recr�er une nouvelle toutes les 0.2s.
        while (true)
        {
    ...
    /// <summary>
    /// Ratio a / (a + b), s�curis� contre la division par z�ro. Si les deux
    /// valeurs sont � 0 (aucun combat encore), retourne 0.5 (barre �
    /// l'�quilibre) plut�t qu'un NaN qui casse le rendu du Canvas.
    /// </summary>
