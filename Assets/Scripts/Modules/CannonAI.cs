...
public class CannonAI : MonoBehaviour
{
    ...
    private float visionConeHalfAngleCos; // Cosinus du demi-angle du c�ne
    private float rangeAttackSqr; // Distance au carr�
    private Vector3 cachedForwardDirection;
    private float detectionCheckTimer = 0f;
    private const float DETECTION_CHECK_INTERVAL = 0.1f; // V�rifier tous les 100ms
    ...
    private void OnEnable()
    {
        ...
        // === OPTIMISATION : Pr�calculer les valeurs ===
    ...
    {
        // === OPTIMISATION : Ne pas v�rifier tous les frames ===
    ...
    public void SetStatistique()
    {
        ...
        // === OPTIMISATION : Mettre � jour les caches ===
        rangeAttackSqr = rangeAttack * rangeAttack;
    }
    ...
    // === OPTIMISATION MAJEURE : DetectTargets ultra-optimis� ===
    private void DetectTargets()
    {
        ...
        for (int i = 0; i < targetCount; i++)
        {
            ...
            // === OPTIMISATION : distance/angle (calculs Vector3 bon march�)
            // v�rifi�s AVANT le GetComponent (bien plus co�teux), pour ne
            ...
            // Early exit si trop loin (garde de s�curit�, normalement d�j�
            ...
            // qui ont d�j� pass� les filtres distance/angle bon march� ===
            if (!col.TryGetComponent(out IDamageable damageable))
                continue;

            if (currenttarget == null)
            {
                // V�rifie la ligne de vue r�elle avant de valider la cible
            ...
            {
                // V�rifie que la ligne de vue vers la cible actuelle est toujours d�gag�e
                Transform validatedTarget = ValidateLineOfSight(origin, col.transform);
                if (validatedTarget != null)
                {
                ...
                // Ligne de vue bloqu�e : on continue � chercher un autre candidat
            }
        }
    ...
    /// <summary>
    /// V�rifie par raycast qu'on peut effectivement toucher la cible candidate.
    /// Si un autre vaisseau ennemi (IDamageable sur targetLayer) est touch�
    /// en premier, celui-ci est retourn� � la place. Retourne null si aucune
    ...
    private Transform ValidateLineOfSight(Vector3 origin, Transform candidate)
    {
        ...
        {
            // La cible candidate est bien ce qui est touch� en premier
            if (hit.transform == candidate || hit.transform.IsChildOf(candidate))
                return candidate;

            // Un autre vaisseau ennemi a �t� touch� avant : on le prend pour cible
        ...
        // Rien touch� sur targetLayer avant la distance de la cible : pas de ligne de vue
        return null;
    }
